...
async function loadRelationOptions(): Promise<OptionMap> {
  const [
    ...
    prisma.knowledgeTrack.findMany({ orderBy: { title: "asc" }, select: { id: true, title: true } }),
    prisma.knowledgeContent.findMany({ orderBy: { title: "asc" }, select: { id: true, title: true } }),
    prisma.quiz.findMany({ orderBy: { title: "asc" }, select: { id: true, title: true } }),
    prisma.quizQuestion.findMany({ orderBy: { prompt: "asc" }, select: { id: true, prompt: true } }),
    prisma.playbookSection.findMany({ orderBy: { title: "asc" }, select: { id: true, title: true } }),
    prisma.commissionBase.findMany({ orderBy: { label: "asc" }, select: { id: true, label: true } }),
    prisma.uploadedAsset.findMany({ orderBy: { createdAt: "desc" }, take: 100, select: { id: true, originalName: true } }),
  ]);

  return {
    product: products.map((item) => ({ label: item.name, value: item.id })),
    productPlan: plans.map((item) => ({ label: item.name, value: item.id })),
    company: companies.map((item) => ({ label: item.tradeName || item.legalName, value: item.id })),
    partnerCompany: partners.map((item) => ({ label: item.tradeName || item.legalName, value: item.id })),
    representative: representatives.map((item) => ({ label: item.name, value: item.id })),
    salesPerson: sellers.map((item) => ({ label: item.name, value: item.id })),
    user: users.map((item) => ({ label: `${item.name} <${item.email}>`, value: item.id })),
    role: roles.map((item) => ({ label: item.name, value: item.id })),
    knowledgeTrack: tracks.map((item) => ({ label: item.title, value: item.id })),
    knowledgeContent: contents.map((item) => ({ label: item.title, value: item.id })),
    quiz: quizzes.map((item) => ({ label: item.title, value: item.id })),
    quizQuestion: questions.map((item) => ({ label: item.prompt.slice(0, 90), value: item.id })),
    playbookSection: playbookSections.map((item) => ({ label: item.title, value: item.id })),
    commissionBase: commissionBases.map((item) => ({ label: item.label, value: item.id })),
    uploadedAsset: assets.map((item) => ({ label: item.originalName, value: item.id })),
  };
}
